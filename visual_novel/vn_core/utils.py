...
class VndbStats:
    ...
    def update_vn(self, vndb_id):
        try:
            response = requests.post("https://api.vndb.org/kana/vn", json={
            ...
            time.sleep(0.1)
        except (IndexError, ValueError, requests.exceptions.RequestException) as exc:
            raise self.VndbAuthError(str(exc)) from exc
